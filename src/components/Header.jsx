...
const Header = () => {
  ...
  return (
    ...
    <div className="max-w-screen-xl mx-auto px-4 flex justify-between items-center py-3">
      ...
      <div>
        <Link to="/">
          <h1 className='text-blue-600 font-semibold text-sm md:text-xl'>Co-Working Space</h1>
        </Link>
      </div>
      ...
      <div className="flex items-center sm:gap-x-6">
        ...
        {user ? (
          <div className="relative">
            ...
            {dropdownOpened && (
              <div className="absolute right-0 mt-2 w-40 bg-white rounded-lg shadow-lg border border-black py-1 z-20">
                <button
                  onClick={handleLogout}
                  className="w-full text-left px-4 py-1 flex items-center text-sm font-medium text-black"
                >
                  <LuLogOut className="w-4 h-4 mr-2 text-blue-600" />
                  <h1 className="text-black">Keluar</h1>
                </button>
              </div>
            )}
          </div>
