...
function Navbar({ containerStyles }) {
  ...
  return (
    <nav className={`${containerStyles} flex justify-around items-center  p-2 rounded-full border border-black`}>
      ...
      <MenuItems
        transition
        className="absolute right-0 z-10 mt-5 w-24 origin-top-right rounded-xl border border-black bg-white shadow-lg ring-1 ring-black ring-opacity-5 transition focus:outline-none data-[closed]:scale-95 data-[closed]:transform data-[closed]:opacity-0 data-[enter]:duration-100 data-[leave]:duration-75 data-[enter]:ease-out data-[leave]:ease-in"
      >
        <div className="py-1">
          <MenuItem>
            <a
              href="/Product"
              className="block px-4 py-2 text-sm text-gray-700 data-[focus]:text-gray-900"
            >
              Produk
            </a>
          </MenuItem>
